...
class ProbAttention(nn.Layer):
    ...
    def _prob_QK(self, Q, K, sample_k, n_top): # n_top: c*ln(L_q)
        ...
        index_sample = paddle.randint(0, L_K, (L_Q, sample_k))  # real U = U_part(factor*ln(L_k))*L_q
        K_sample = K_expand[:, :, paddle.arange(L_Q).unsqueeze(1), index_sample, :]

        index_long = index_sample.reshape((-1, 1)).squeeze(1)  # (48*12)
        # print(index_long.device, K.device)
        K_sample2 = paddle.index_select(x=K, axis=2, index=index_long)  # (32,8,48*12,64)
        print(K_sample2.shape)
        K_sample2 = K_sample2.transpose((0,1,3,2)).reshape(B,H,E,L_K,-1).transpose((0,1,3,4,2))
        print(K_sample2.shape)

        Q_K_sample = paddle.matmul(Q.unsqueeze(-2), K_sample.transpose(-2, -1)).squeeze(-2)

        # find the Top_k query with sparisty measurement
        M = Q_K_sample.max(-1) - paddle.divide(Q_K_sample.sum(-1), paddle.to_tensor([L_K])*1.0)
        ...
        #              M_top, :] # factor*ln(L_q)
        Q = Q.reshape((-1, L_Q, E))  # (256,48,64)
        M_top_index = M_top.reshape(-1, n_top)  # (256,12)
        Q_reduce = paddle.concat(
            [paddle.index_select(Q[i], axis=0, index=M_top_index[i]).unsqueeze(0) for i in range(Q.shape[0])]
            # (1, 12, 64)
        ) # (256, 12, 64)
        Q_reduce = Q_reduce.reshape((B, H, n_top, E))
